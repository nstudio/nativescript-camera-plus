...
import { CameraPlusBase, GetSetProperty, CameraUtil, CLog, ICameraOptions, IChooseOptions, ICameraPlusEvents, IVideoOptions, CameraTypes } from './camera-plus.common'
...
export class MySwifty extends SwiftyCamViewController {
    ...
    public static initWithOwner(owner: WeakRef<CameraPlus>, enableVideo: boolean = false, defaultCamera: CameraTypes = 'rear') {
    ...
    public tookPhoto(photo: UIImage) {
        ...
        if (this._snapPicOptions && this._snapPicOptions.confirm) {
            ...
            let retakeBtn = createButton(this, CGRectMake(10, 10, 75, 50), 'Retake', 'resetPreview');
            let saveBtn = createButton(this, CGRectMake(width - 170, 10, 150, 50), 'Save', 'savePhoto', 'right');
    ...
    private _addButtons() {
        ...
        if (this._owner.get().showToggleIcon) {
            ...
            let switchCameraBtn = createButton(this, CGRectMake(width - 100, 20, 100, 50), null, 'switchCam', null, createIcon('toggle', CGSizeMake(65, 50)));
            switchCameraBtn.transform = CGAffineTransformMakeScale(.75, .75);
        ...
        if (this._owner.get().showGalleryIcon === true) {
            let galleryBtn = createButton(this, CGRectMake(20, height - 80, 50, 50), null, 'openGallery', null, createIcon('gallery'));
            galleryBtn.transform = CGAffineTransformMakeScale(.75, .75);
            this.view.addSubview(galleryBtn);
        }

        if (this._owner.get().showCaptureIcon) {
            let picOutline = createButton(this, CGRectMake((width / 2) - 20, height - 80, 50, 50), null, null, null, createIcon('picOutline'));
            picOutline.transform = CGAffineTransformMakeScale(1.5, 1.5);
            this.view.addSubview(picOutline);
            let takePicBtn = createButton(this, CGRectMake((width / 2) - 21.5, height - 80.7, 50, 50), null, this._enableVideo ? 'recordVideo' : 'snapPicture', null, createIcon('takePic'));
    ...
    private _flashBtnHandler() {
        if (this._owner.get().showFlashIcon) {
            ...
            this._flashBtn.transform = CGAffineTransformMakeScale(.75, .75);
